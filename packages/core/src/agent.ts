...
function scanIssue(issue: Issue, deploys: Deploy[], runbooks: Runbook[]): IncidentFinding {
  ...
  if (issue.id === "PROD-4521") {
    ...
    const evidence = [
      issueEvidence(issue, "Stack trace hits PaymentService.java:142 and /checkout p99 rose to 8.2s."),
      deployEvidence(
        deploy,
  ...
  if (issue.id === "PROD-4487") {
    ...
    const runbook = mustFindRunbook(runbooks, "payment-service-degraded");
    const evidence = [
      issueEvidence(issue, "Acme Corp has 2,400 seats and reports tenant-specific checkout failure."),
      deployEvidence(
        deploy,
    ...
    return incidentFinding({
      ...
      confidence: 0.92,
      evidence,
      hypothesis: "Acme is in the guest-checkout cohort and is hitting the payment regression path.",
      issue,
      rankScore: 88,
      recommendedAction:
  ...
  if (issue.id === "PROD-4498") {
    ...
    const evidence = [
      issueEvidence(issue, "502 windows last 25-35 seconds and load balancer logs show upstream timeouts."),
      runbookEvidence(
        runbook,
  ...
  if (issue.id === "PROD-4519") {
    ...
    const evidence = [
      issueEvidence(issue, "Uploads take 30-60 seconds on iOS and Android while backend logs show latency."),
      deployEvidence(deploy, "Recent signing-service deploy reduced signed URL TTL."),
      runbookEvidence(runbook, "All-region upload latency points back to signed URL TTL, not the CDN.")
    ];
...
function buildActionDraft(issue: Issue, severity: Severity, recommendedAction: string): ActionDraft {
